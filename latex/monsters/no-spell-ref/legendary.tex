...
\begin{DndMonster}[float*=hb,width=\textwidth + 8pt]{Lich}
	...
	\begin{itemize}
		\item[] \textbf{Concentration}: \textit{maze} (Legendary, 1x), \textit{antilife shell} (1x), \textit{domination} (Legendary, 1x)
		\item[] \textbf{Damage}: \textit{power word kill} (Legendary, use level of 20, 1x), \textit{disintegrate} (Legendary, 2x), \textit{rip soul} (Legendary, 2x), \textit{circle of death} (2x)
		\item[] \textbf{Defensive}: \textit{misty step} (at will), \textit{blink} (1x)
	\end{itemize}
...
\begin{itemize}
	\item[] \textbf{Concentration} \textit{arcane binding} (2x), \textit{shield of faith} (1x)
	\item[] \textbf{Damage} \textit{harm} (Legendary, 1x, 14d6+30), \textit{guiding bolt} (3x)
	\item[] \textbf{Other} \textit{unbind} (3x), \textit{insect plague} (1x)
\end{itemize}
\DndMonsterSection{Actions}
