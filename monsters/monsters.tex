...
\begin{figure}
	\begin{DndComment}{Expected PC Damage and accuracy}
		...
		\begin{itemize}
			...
			\item After accounting for accuracy, a not-very-optimized \textit{party} is expected to deal on average 3x the damage of a similar-level rogue with a shortbow who gets sneak attack each round and hits (including crits) 70\% of the time. This rogue-based reference point is refered to as Rogue Equivalent Damage (RED). The variance in party damage is expected to be roughly between 2.5 RED and ~4-5 RED, depending on resource usage, targets, etc. This results in monsters generally surviving around 3 rounds, except at level 1 where a DR 1 monster survives roughly 2 rounds. Note that a damage-focused character normally does a baseline of ~1.2-1.5 RED (with spikes up to 2+ RED when burning resources) and a spell-caster using cantrips ends up around ~0.5 RED.
	\end{DndComment}
\end{figure}
