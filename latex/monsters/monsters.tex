...
A monster can be Diminutive, Tiny, Small, Medium, Large, Huge, or Gargantuan. The Size Categories table shows how much space a creature of a particular size controls in combat. See the Core Rules for more information on creature size and space.

\begin{DndTable}[header=Size Categories]{lXr}
    \textbf{Size}       & \textbf{Space}                  & \textbf{Examples}    \\
		Diminutive & ---										& Most normal vermin and insects \\       
...
\textbf{Aberrations} are utterly alien beings. Many of them have innate magical abilities drawn from the creature's alien mind rather than the mystical forces of the world. The quintessential aberrations are the twisted comiedai, mkhulu, and Awakened corruptions.

\textbf{Beasts} are non-humanoid creatures that are a natural part of the fantasy ecology. Some of them have magical powers, but most are unintelligent and lack any society or language. Beasts include all varieties of ordinary animals, dinosaurs, and giant versions of animals.
...
\textbf{Constructs} are made, not born. Some are programmed by their creators to follow a simple set of instructions, while others are imbued with sentience and capable of independent thought. Golems are the iconic constructs.
...
\textbf{Giants} tower over humans and their kind. They are human-like in shape, though some have multiple heads (ettins) or deformities (fomorians). The six varieties of true giant are hill giants, stone giants, frost giants, fire giants, cloud giants, and storm giants. Besides these, creatures such as ogres and trolls are giants.
...
Monsters have an attitude listed next to their size and type (e.g. \textit{Large beast, craven}) that describes how a generic member of that type of monster will act by default. Specific individuals may act differently, but this serves as a guide to role play for the GM. Example attitudes:
...
\subparagraph*{Fearless} Fearless monsters are immune to morale checks. While not totally single-minded, they generally are resistant to mental manipulation.

\subparagraph*{Guardian} Guardian monsters have either been trained to protect their masters or are naturally inclined to protect their fellows/allies. Usually combined with domestic. Guardian animals act as lookouts and will aggressively intervene in attempts to attack their protected allies.

\subparagraph*{Domestic} Domesticated monsters are particularly apt to be used as beasts of burden, mounts, work animals, and pets. In general, they will avoid fighting unless trained as a guard animal, and can be easily soothed with Animal Handling checks.

\subparagraph*{Herd Mentality} Herd animals are a mix of pack animals and craven. they operate together, and if one fights they all fight; if one flees, the rest will stampede as well.
...
\subparagraph*{Group/Pack Mentality} The opposite of being Individualistic. These bond to their pack and will protect them, up to sacrificing themselves for other members of their pack.
...
A monster that wears armor or carries a shield usually has an Armor Class (AC) that takes its armor, shield, and Dexterity into account. Otherwise, a monster's AC is based on its Dexterity modifier and natural armor, if any, but may include other modifiers to have the appropriate fictional and mechanical behavior. If a monster has natural armor, wears armor, or carries a shield, this is noted in parentheses after its AC value.

\subsection{Hit Points}

A monster usually dies or is destroyed when it drops to 0 hit points. For more on hit points, see \smartnameref{ch:character-creation}{Character Creation in the main rules}.
...
\begin{figure}
	\begin{DndComment}{Variant: Monster Death Saves and Permanent Injuries}
		As stated, the default is that monsters die or are destroyed when they reach 0 hit points rather than having death saves like a player character. As the GM, you can choose to allow significant monsters (often those with player-visible names, rather than the various mooks and minions) to make death saving throws, following the same rules as PCs. This allows both their allies to attempt to save them (prolonging the fight or providing a chance for them to escape) and the party to choose to save them for questioning more efficiently.

		It is thematically appropriate to have monsters acquire scars or permanent injuries when they survive something that normally should have killed them. PCs, as the fate-blessed protagonists, shouldn't suffer such injuries generally. Unless your table decides they find them more fun that way.
	\end{DndComment}
\end{figure}
...
A monster's speed tells you how far it can move on its turn. For more information on speed, see \smartnameref{ch:order-of-combat}{Order of Combat in the main rules}.
...
A monster that has a burrowing speed can use that speed to move through sand, earth, mud, or ice. A monster can't burrow through solid rock unless it has a special trait that allows it to do so. Burrowing out of reach provokes opportunity attacks. Burrowing doesn't leave a usable tunnel behind it unless the monster has a special trait that says otherwise (such as the purple worm).

\subsubsection{Climb}

A monster that has a climbing speed can use all or part of its movement to move on vertical surfaces. The monster doesn't need to spend extra movement to climb. Monsters generally do still need a free hand to climb unless they have an ability such as Spider Climb that says otherwise.
...
A monster that has a swimming speed doesn't need to spend extra movement to swim. They are also immune to any of the penalties of fighting underwater (such as disadvantage on melee attacks, etc.).

\subsection{Ability Scores and Saving Throws}

Every monster has six ability scores (Strength, Dexterity, Constitution, Intelligence, Wisdom, and Charisma). If a monster has a bonus to a saving throw, it is listed in the saving throws section. If a save isn't listed, the saving throw modifiers are equal to the ability scores. 

\subsubsection{Morale}

Monsters, unlike PCs, can be \nameref{condition:broken} by effects that shake them enough emotionally. When the GM decides that a non-legendary monster may not be able to continue a combat for reasons other than being reduced to zero hit points (or outright killed), they will generally make a Morale save for that creature. This is a Wisdom saving throw. The first save usually has a DC of 10; particularly dramatic events may require beating a DC of 15 to succeed. On a failed save, the creature gains the \nameref{condition:broken} condition until conditions materially change or the end of their next turn, whichever lasts longer.

\subparagraph*{Triggering a Morale Save}
Here are some conditions that might (depending on the fiction) trigger a Morale Save.
\begin{itemize}
	\item A hunting predator becoming \nameref{condition:bloodied} (unless it is defending its nest or magically compelled).
	\item A common warrior (guard, goblin raider, etc.) seeing their leader get taken out.
	\item A common warrior seeing a bunch of their allies killed in a single turn (such as by a powerful area of effect ability).
	\item One of their more powerful allies being downed in a single turn.
	\item Powerful supernatural or monstrous fear effects against "normal" people.
	\item A powerful intimidation attempt or offer of surrender when already on the edge.
\end{itemize}

\subparagraph*{Recovering from Being Broken}
A few possible circumstances that may trigger a recovery from being broken:
\begin{itemize}
	\item A powerful enemy being reduced to zero hit points.
	\item A rousing speech by a leader they trust.
	\item A supernatural intervention on their side (such as a demon being summoned to reinforce cultists)
	\item Reinforcements or news of a victory elsewhere.
\end{itemize}
...
Some creatures have vulnerability, resistance, or immunity to certain types of damage. In addition, some creatures are immune to certain conditions. Immunity to a condition such as poisoned does not imply immunity to a matching damage type. The reverse is also not implied---immunity to poison damage does not imply resistance or immunity to the poisoned condition.
...
A monster with blindsight can perceive its surroundings without relying on sight, within a specific radius. For all purposes this counts as being able to see within that radius. Creatures without eyes, such as grimlocks and gray oozes, typically have this special sense, as do creatures with echolocation or heightened senses, such as bats and true dragons. If a monster is naturally blind other than its blindsight, it has a parenthetical note to this effect, indicating that the radius of its blindsight defines the maximum range of its perception.
...
A monster with truesight can, out to a specific range, see in normal and magical darkness, see invisible creatures and objects (completely negating the invisible condition with respect to that monster), automatically detect visual illusions and succeed on saving throws or ability checks against them, and perceive the original form of a shapechanger or a creature that is transformed by magic. Furthermore, the monster can see into the Ethereal Plane within the same range.

\subparagraph*{Armor, Weapon, and Tool Proficiencies}

Assume that a creature is proficient with its armor and tools. If you swap them out, you decide whether the creature is proficient with its new equipment. Except in rare cases, monsters generally prefer to use weapons and armor suited for them. For example, a hill giant typically wears hide armor and wields a greatclub. You could equip a hill giant with chain mail and a greataxe instead, and assume the giant is proficient with both, one or the other, or neither. See the \smartnameref{ch:equipment}{Equipment in the main rules} for rules on using armor without proficiency.

Monsters conditionally apply their additional bonus to attack rolls, as described above. This same bonus should apply to all attacks if it applies to any, and the game goes more smoothly if all relevant numeric bonuses are listed in the monster's attack entry.
...
Telepathy is a magical ability that allows a monster to communicate mentally with another creature within a specified range. The contacted creature doesn't need to share a language with the monster to communicate in this way with it, but it must be able to understand at least one language. A creature without telepathy can receive and respond to telepathic messages but can't initiate or terminate a telepathic conversation. A creature in any location where magic doesn't function can't send or receive telepathic messages.

A telepathic monster doesn't need to see a contacted creature and can end the telepathic contact at any time. The contact is broken as soon as the two creatures are no longer within range of each other or if the telepathic monster contacts a different creature within range. A telepathic monster can initiate or terminate a telepathic conversation without using an action, but while the monster is incapacitated, it can't initiate telepathic contact, and any current contact is terminated.

\subsection{Challenge}

A monster's \textbf*{challenge ratings} tells you how great a threat the monster is. Each monster has two ratings, offensive and defensive, separated by a slash, written as "offensive/defensive". Generally, the overall rating is the geometric mean of the two (the square root of the product of the values), but this really provides little relevant information.

\subparagraph*{Offensive Rating (OR)} The offensive rating of a monster relative to the party level tells you how likely it is for that monster to drop a full-health character to 0 HP in a single round.
...
\subparagraph*{Defensive Rating (DR)} The defensive rating of a monster tells you how likely it is for that monster to survive long enough under direct fire from the party's primary damage dealers in order to do use its cool abilities at least once (generally considered about 3 rounds). This assumes "baseline" characters without substantial combat-effective magic items or particularly targeting vulnerabilities. It's a rough "static training dummy" measure and should be taken with a grain of salt.

\begin{itemize}
	\item[] \textbf{DR > Level} The monster is likely to survive at least 3 rounds of direct fire from a baseline party.
	\item[] \textbf{DR $\approx$ Level} The monster will probably die in 2-3 rounds of direct fire from a baseline party, or survive double that if party attention is split between it and other monsters.
...
A monster with the Spellcasting special trait has the ability to cast selected spells, which it uses to cast its spells (as explained in \smartnameref{ch:spellcasting}{Spellcasting in the main rules}).
...
\begin{DndComment}{Spell selection}
    Casters generally only have a limited selection of spells in their stat blocks, those focused on combat use. Generally they have a big concentration spell and one or more lower priority or defensive spells. Their primary spell damage comes from their spell attacks, which count as cantrips. Non-combat spells are at the discretion of the GM and should be chosen as appropriate for the fictional position of that particular caster.

	Be wary of hard crowd control spells on high-Constitution casters. Hard crowd control spells are those that impose conditions such as stunned or paralyzed that completely deny PC turns. Many tables consider those "unfun" since the affected party member cannot do anything. In addition, these spells and abilities can swing fights tremendously---landing one early on a key damage dealer or support character can mean a party's total defeat while not landing it at all can result in a cakewalk fight. Effectively, these spells are extremely hard to judge as to effectiveness and produce wild swings in table experience. Soft crowd control such as \smartnameref{spell:slow}{slow} isn't nearly as swingy.
\end{DndComment}

\subsection{Actions}

When a monster takes its action, it can choose from the options in the Actions section of its stat block or use one of the actions available to all creatures, such as the Dash or Hide action, as described in \smartnameref{ch:order-of-combat}{Order of Combat in the main rules}.

\subsubsection{Melee and Ranged Attacks}

The most common actions that a monster will take in combat are melee and ranged attacks. These can be spell attacks or weapon attacks, where the “weapon” might be a manufactured item or a natural weapon, such as a claw or tail spike. For more information on different kinds of attacks, see \smartnameref{ch:order-of-combat}{Order of Combat in the main rules}.
...
\subparagraph*{Hit} Any damage dealt or other effects that occur as a result of an attack hitting a target are described after the “\textit{Hit}” notation. You have the option of taking average damage or rolling the damage; for this reason, both the average damage and the die expression are presented. Any effect listed in this portion takes place only if the attack hits. Only damage that doesn't require an additional saving throw to take effect after a hit is doubled on a critical hit.
...
Some monsters have spell attacks. These count as cantrips for features for that care about spells. Generally, they do not require material components (but require one of verbal or somatic components unless specified otherwise) and are specialized for that creature. 

\subsection{Multiattack}

A creature that can make multiple attacks on its turn has the Multiattack action. A creature can't use Multiattack when making an opportunity attack, which must be a single melee attack. If the creature has the Multiattack ability, it specifies exactly which attacks may be made and in what combinations.
...
All creatures with a melee attack in their stat block can make opportunity attacks. Monsters cannot take the special actions outlined in \smartnameref{sec:stamina-and-aether}{Stamina and Aether in the main rules}. If they have similar capabilities, those will appear as explicit actions in their stat blocks. 
...
You can equip monsters with additional gear and trinkets however you like, and you decide how much of a monster's equipment is recoverable after the creature is slain and whether any of that equipment is still usable. A battered suit of armor made for a monster is rarely usable by someone else, for instance. It's normally rare for non-magical weapons used by monsters to have much value.

If a spellcasting monster needs material components to cast its spells, assume that it has the material components it needs to cast the spells in its stat block.

\subsection{Variants}

Some monsters have variant options, such as the acolyte having the cultist variant. Variants are listed in their own section and generally add traits or actions or modify the basic statistics. Each variant describes how it affects the challenge ratings of the monster when applied.
...
\subparagraph*{Lineage and Cultural Traits} You can add lineage and cultural traits to an NPC (especially humanoids and giants). Adding racial traits to an NPC doesn't alter its challenge rating. For more on lineage traits, see \smartnameref{ch:lineages}{Lineages in the main rules}; for more on cultural traits, see \smartnameref{sec:cultures}{Cultures in the main rules}.
...
\begin{DndComment}{Non-damaging spells and abilities and challenge ratings}
	...
	\begin{itemize}
		...
		\item It a spell has a long duration (ie > 10 minutes) and does not require concentration, assume that it has pre-cast that spell unless the monster is surprised. Include such things as \smartnameref{spell:mage-armor}{\textit{mage armor}} directly in the stat block as a baseline.
	\end{itemize}
\end{DndComment}
...
\begin{enumerate}
	...
	\item Compare to the Offensive Rating table and find the matching rating. Note the attack bonus and saving throw modifier listed there.
...
\begin{DndTable}[header=Offensive Rating]{llllll}
	\textbf{OR} & \textbf{Max Bonus} & \textbf{Damage Range} & \textbf{Attack} & \textbf{DC} \\
	0 & 2 & 0-1 & +2 & 10 \\
	1/8 & 2 & 2-3 & +2 & 10 \\
	1/4 & 2& 4-5 & +3 & 11 \\
	1/2 & 2& 6-7 & +3 & 11 \\
...
\begin{itemize}
	\item calculate the base hit points of the monster, or pick an appropriate number from the Defensive Rating table table for the rating you approximately wish to end up with.
...
\begin{DndTable}[header=Defensive Rating]{llllll}
...
\begin{DndTable}[header=DPR by level]{ll}
