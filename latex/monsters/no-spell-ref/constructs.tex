...
Generally, constructs are immune to most mind-or-soul affecting abilities as well as poison, but repairing them is more difficult as they do not heal naturally and most healing magic does not fix them. As a general rule, constructs only become disabled when their animating force (regardless of its nature) is damaged or dispelled. Those animated directly by spells, such as the Animated Objects below, are disrupted by magic such as \textit{unbind}, while those that operate via clockwork or trapped spirits are not.
...
\begin{DndMonster}{Animated Furniture}
	...
	\DndMonsterAction{Antimagic Susceptibility} The construct is incapacitated while in the area of \textit{abjure aether manipulation}. If targeted by \textit{unbind}, the construct must succeed on a Constitution saving throw against the caster's spell save DC or fall unconscious for 1 minute.
...
\begin{DndMonster}{Animated Armor}
	...
	\DndMonsterAction{Antimagic Susceptibility} The armor is incapacitated while in the area of \textit{abjure aether manipulation}. If targeted by \textit{unbind}, the armor must succeed on a Constitution saving throw against the caster's spell save DC or fall unconscious for 1 minute.
...
\begin{DndMonster}[width=\textwidth + 8pt]{Golem, Clay}
	...
	\DndMonsterAttack[
		...
		dmg=\DndDice{2d10 + 5},
		dmg-type=bludgeoning,
		extra={. If the target is a creature, it must succeed on a DC 13 Constitution saving throw or have its hit point maximum reduced by an amount equal to the damage taken. The target dies if this attack reduces its hit point maximum to 0. The reduction lasts until removed by the Restoration incantation (in its greater form) or other magic of similar power.}
	]
...
This counts as being the spell \textit{slow} but does not have any components.
\end{multicols}
\end{DndMonster}
